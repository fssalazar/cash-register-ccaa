...
export function CashRegister({ cashRegister, sessions }: Props) {
  ...
  const isSessionOpen = useMemo(() => {
    if (!sessions || sessions.length === 0) {
      return false; // No sessions exist, so no session is open
    }
    // Sort sessions by openDate descending to get the latest session
    const sortedSessions = [...sessions].sort(
      (a, b) => new Date(b.openDate).getTime() - new Date(a.openDate).getTime()
    );
    const latestSession = sortedSessions[0];
    return !latestSession.closeDate; // If closeDate is not set, the session is open
  }, [sessions]);
